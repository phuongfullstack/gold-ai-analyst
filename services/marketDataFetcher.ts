...
const PNJ_API = 'https://giavang.pnj.com.vn/api/giavang/list'; // PNJ often blocks but worth having
...
const normalizePrice = (val: number) => {
  if (val <= 0) return 0;
  // Case 1: Raw VND per tael (e.g. 172,000,000)
  if (val > 1000000) return val / 1000000;
  // Case 2: Thousand VND per tael (e.g. 85,000 or 172,000)
  // Threshold 30,000: gold tael is always > 30M and gold chi is < 25M
  if (val > 30000) return val / 1000;
  // Case 3: Thousand VND per chi (e.g. 8,500 or 17,150)
  // 17,150 (k VND/chi) * 10 chi/tael / 1000 k/M = 171.5 M/tael (val / 100)
  if (val > 1000) return val / 100;
  // Case 4: Already Million VND per tael (e.g. 172.0)
  return val;
};

export const fetchSjcXmlData = async (): Promise<Partial<MarketData>> => {
  try {
    // Try both HTTPS and HTTP just in case
    const sjcRes = await fetch(proxyUrl(SJC_XML));
    if (sjcRes.ok) {
      const xmlText = await sjcRes.text();
      if (!xmlText.includes('<ratelist')) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if ((type.includes("Nhẫn") || type.includes("99.99")) && !result.ringGoldSell) {
  ...
  try {
    ...
    if (dojiRes.ok) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (name.includes("Nhẫn") || name.includes("9999") || name.includes("Hưng Thịnh Vượng") || name.includes("Lộc Phát Tài")) {
        ...
        if (name.includes("Vàng 24k") && !result.ringGoldSell) {
          result.ringGoldBuy = normalizePrice(buy);
          result.ringGoldSell = normalizePrice(sell);
        }
      }
      ...
      rows.forEach(row => {
        ...
        if (cells.length >= 3) {
          ...
          if (buy > 0) {
            ...
            } else if ((text.includes("nhẫn") || text.includes("9999") || text.includes("24k")) && !result.ringGoldSell) {
