...
const App: React.FC = () => {
  ...
  const toggleChartPlaceholder = (show: boolean, reportData: AnalysisReport | null, mData: MarketData | null) => {
    ...
    if (show && reportData && mData) {
      ...
      const sentiment = (sigs.confidenceScore?.label || 'TRUNG LẬP').replace(/[<>]/g, '');
      ...
      const priceForCalc = mData.xauPrice > 0 ? mData.xauPrice : 2500;
      let lastPrice = priceForCalc;
      const bars = Array.from({ length: 24 }).map((_, i) => {
          const change = (Math.random() - 0.48) * (priceForCalc * 0.01); // ~1% volatility
          const open = lastPrice;
          const close = open + change;
          const high = Math.max(open, close) + Math.random() * (priceForCalc * 0.002);
          const low = Math.min(open, close) - Math.random() * (priceForCalc * 0.002);
          lastPrice = close;
          ...
          const volatilityFactor = priceForCalc * 0.02;
          const bodyHeight = Math.max(2, Math.abs(close - open) / volatilityFactor * range);
          const wickTopHeight = (high - Math.max(open, close)) / volatilityFactor * range;
          const wickBottomHeight = (Math.min(open, close) - low) / volatilityFactor * range;
